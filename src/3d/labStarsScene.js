...
/**
 * =========================================================
 * MINI PREVIEWS DE LAS CARDS
 * =========================================================
 */
export function initLabPreviewScenes() {
 const canvases = document.querySelectorAll("[data-preview-id]");
 ...
 canvases.forEach((canvas) => {
  const previewId = canvas.dataset.previewId;
  let cleanup = null;

  if (previewId === "preview-main") {
   cleanup = createMainPreview(canvas);
  } else if (previewId === "preview-galaxy") {
   cleanup = createGalaxyPreview(canvas);
  } else if (previewId === "preview-object") {
   cleanup = createObjectPreview(canvas);
  }
...
function createGalaxyPreview(canvas) {
 const scene = new THREE.Scene();
 ...
 const insideColor = new THREE.Color("#fb923c");
 const outsideColor = new THREE.Color("#dbeafe");
 ...
 for (let i = 0; i < count; i++) {
  ...
  const spinAngle = radius * 2.3;

  const randomX = (Math.random() - 0.5) * 0.18 * radius;
  const randomY = (Math.random() - 0.5) * 0.08;
  const randomZ = (Math.random() - 0.5) * 0.18 * radius;
  ...
  mixedColor.lerp(outsideColor, radius / 2.2);
 ...
 points.rotation.y = 0.28;
 points.position.y = -0.42;
 scene.add(points);

 const ambientLight = new THREE.AmbientLight("#ffffff", 0.7);
 scene.add(ambientLight);

 const glowLight = new THREE.PointLight("#fb923c", 5, 10);
 ...
 const handleMouseMove = (event) => {
  ...
  targetRotationY = 0.28 + (x - 0.5) * 0.35;
  targetRotationX = -0.42 + (y - 0.5) * 0.05;
 };
 ...
 const tick = () => {
  ...
  points.rotation.y += (targetRotationY - points.rotation.y) * 0.03;
  points.rotation.x += (targetRotationX - points.rotation.x) * 0.03;

  points.position.x = Math.sin(elapsed * 0.8) * 0.04;
  points.position.y = -0.42 + Math.sin(elapsed * 0.7) * 0.015;
...
function createObjectPreview(canvas) {
 const scene = new THREE.Scene();
 ...
 const core = new THREE.Mesh(new THREE.IcosahedronGeometry(0.85, 0), coreMaterial);
 group.add(core);

 const ringOne = new THREE.Mesh(new THREE.TorusGeometry(1.45, 0.04, 16, 100), ringMaterial);
 ringOne.rotation.x = 0.9;
 group.add(ringOne);

 const ringTwo = new THREE.Mesh(new THREE.TorusGeometry(1.1, 0.035, 16, 100), ringMaterial);
 ringTwo.rotation.y = 0.8;
 ringTwo.rotation.z = 0.45;
 group.add(ringTwo);

 const particles = createMiniStars({
  count: 100,
  spreadX: 5,
  spreadY: 4,
  spreadZ: 4,
 ...
 particles.points.position.y = 0.1;
 ...
 const accentLight = new THREE.PointLight("#fb923c", 8, 20);
 accentLight.position.set(-2, 1.5, 2);
 ...
 const tick = () => {
  ...
  ringOne.rotation.z += 0.01;
  ringTwo.rotation.x += 0.008;

  group.position.y = -0.46 + Math.sin(elapsed * 1.3) * 0.04;
  particles.points.rotation.y = elapsed * 0.05;

  ringMaterial.emissiveIntensity = 0.12 + Math.sin(elapsed * 2.2) * 0.05;
